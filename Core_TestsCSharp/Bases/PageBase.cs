...
namespace Core_TestsCSharp.Bases
{
    class PageBase
    {
        ...
        protected IWebDriver driver { get; private set; }  //Otimiza o uso do INSTANCE estático
        protected IJavaScriptExecutor javaScript { get; private set; }

        public PageBase()
        {
            wait = new OpenQA.Selenium.Support.UI.WebDriverWait(DriverFactory.INSTANCE, TimeSpan.FromSeconds(Convert.ToDouble(ConfigurationManager.AppSettings["TimeOut"].ToString())));
        ...
        protected void Click(By locator)
        {
            ...
            while (timeOut.Elapsed.Seconds <= Convert.ToInt32(ConfigurationManager.AppSettings["TimeOut"].ToString()))
            {
                try
                {
